...
{
 	var keyword = document.detailForm.searchKeyword;
	var condition = document.detailForm.searchCondition;

	if (!keyword.value)
	{
		condition.value = null;
	}
	else if (isNaN(Number(keyword.value)))
	{
...
{
	var searchCon = document.detailForm.searchCondition;
	var searchKey = document.detailForm.searchKeyword;
	...
	{
		searchCon.value = "${search.searchCondition}"
	}
}
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td background="/images/ct_btnbg02.gif" class="ct_btn01" style="padding-top:3px;">
						<a href="javascript:fncDetailFormSubmit();">검색</a>
					</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<c:forEach var="product" items="${list}">
		...
		<tr class="ct_list_pop">
			...
			<td></td>			
			<td align="left">
				<c:if test="${ param.menu=='search' && product.proTranCode=='3' }">
					재고 없음
				</c:if>
				<c:if test="${ param.menu!='search' || product.proTranCode!='3' }">
					${product.proTranCodeStr}
				</c:if>
...
<script>updateSearchCondition()</script>

	<c:if test="${ resultPage.totalCount > 0 }">
		<jsp:include page="/common/pageNavigator.jsp">
			<jsp:param name="hrefUrl" value="/product/listProduct?menu=${param.menu}"/>
		</jsp:include>
	</c:if>
